...
def CropSUFwithBAC(diretorio,
    ...
    for x in SliceRange:
        if x < 10:
            a=io.imread(diretorio + ImportstackRootName + '/t' + str(SUFstackNumber) + '/Slice00' + str(x) + importFormat)
            
        elif 10 <= x < 100:
            a=io.imread(diretorio + ImportstackRootName + '/t' + str(SUFstackNumber) + '/Slice0' + str(x) + importFormat)

        elif 100 <= x < 1000:
            a=io.imread(diretorio + ImportstackRootName + '/t' + str(SUFstackNumber) + '/Slice' + str(x) + importFormat)            
    ...
    print('\n Iniciando o CropRegion module para o stack ', str(BACstackNumber), ' ... \n')
            ...
            if (Ymin-AreaBorderSize) <= 0 or (Ymax+AreaBorderSize) >= len(RefP3) or (Xmin-AreaBorderSize) <= 0 or (Xmax+AreaBorderSize) >= len(RefP3[0]):
                ...
                try:
                    for imgN in list(range(len(ImgArray3D))):
                        ...
                        if not os.path.exists(diretorio + ExportDir + '/With_BAC/t' + str(BACstackNumber) + '/Region' + str(indexToSave)):
                            os.makedirs(diretorio + ExportDir + '/With_BAC/t' + str(BACstackNumber) + '/Region' + str(indexToSave))
            
                        io.imsave(diretorio + ExportDir + '/With_BAC/t' + str(BACstackNumber) + '/Region' + str(indexToSave) + '/Slice' + str(SliceRange[imgN]) + importFormat, aa1F)
                    
                        if not os.path.exists(diretorio + '/Imagens'):
                            os.makedirs(diretorio + '/Imagens')
                        
                        try:
                            RefImgwithRegOverlay=io.imread(diretorio + '/Imagens/ZProjection_SUFCropped_t' + str(BACstackNumber) + importFormat)
                        ...
                        io.imsave(diretorio + '/Imagens/ZProjection_SUFCropped_' + 't' + str(BACstackNumber) + importFormat, RefImgwithRegOverlay)
                    ...
                    io.imsave(diretorio + ExportDir + '/With_BAC/t' + str(BACstackNumber) + '/Region' + str(indexToSave) + '/ZProjectionBAC' + importFormat, RefCIm2)
                    ...
                    if Det_TopoXVol_relation == True:
                        ...
                        with open(diretorio + ExportDir + '/With_BAC/t' + str(BACstackNumber) + '/Region' + str(indexToSave) + '/Bac_Vol.txt', 'w') as BacVOL_FILE:
...
def CropSUFwithoutBAC(diretorio,
    ...
    print('\n Importando a imagem referencia (com bacterias)... \n')
            ...
            for CropYpos in list(range(len(CropYposList))):
                for CropXpos in list(range(len(CropXposList))):
                    ...
                    try:
                        ...
                        if not os.path.exists(diretorio + ExportDir + '/WithoutBACScaled' + '/t' + str(SUFstackNumber) + '/Region' + str(RegionNumber)):
                            os.makedirs(diretorio + ExportDir + '/WithoutBACScaled' + '/t' + str(SUFstackNumber) + '/Region' + str(RegionNumber))
                        
                        io.imsave(diretorio + ExportDir + '/WithoutBACScaled' + '/t' + str(SUFstackNumber) + '/Region' + str(RegionNumber) + '/Slice' + str(SliceRange[imgN]) + importFormat, aaf1)
                        RegionNumber += 1
...
def TopographicAnalysisFunc(diretorio,
                            ...
                            BACFirstRegion=1,BACLastRegion=2,
                            WithoutBACFirstRegion=1,WithoutBACLastRegion=2,
    ...
    if RegionAnalysis == False:
        print('\n O Modo regionAnalysis estah OFF \n')
        print('\n A topografia da imagem inteira será analisada \n')
        SliceRange=list(range(FirstSlice,LastSlice+1))
            
        Imglist=[]
        for sliceN in SliceRange:
            if sliceN < 10:
                a11=io.imread(diretorio + importstackRootName + '/t' + str(SUFstackNumber) + "/Slice00" + str(sliceN) + ".png")
                
            elif sliceN < 100:
                a11=io.imread(diretorio + importstackRootName + '/t' + str(SUFstackNumber) + "/Slice0" + str(sliceN) + ".png")
            
            elif sliceN < 1000:
                a11=io.imread(diretorio + importstackRootName + '/t' + str(SUFstackNumber) + "/Slice" + str(sliceN) + ".png")
        ...
        print('As dimensoes da matrix analisada sao ', len(HeighvalYX), len(HeighvalYX[0]))            
        ...
        TOPOResults.write('\n O valor de Sa para a Area Total eh ' + str(SaV) + '\n')
        TOPOResults.write('\n O valor de Sq para a Area Total eh ' + str(SqVal)+ '\n')
        TOPOResults.write('\n O valor de Ssk para a Area Total eh ' + str(SskVal) + '\n')
        TOPOResults.close()
        
        imHeights=np.array(HeighvalYX,dtype='uint8')
        io.imsave(diretorio + '/Imagens/HeightsImage_get' + str(heightValuetoGet) + '.png', imHeights)
        ...
        plt.tight_layout()
        figTOPOFull.savefig(diretorio + '/Imagens/HeightsImageMap_get' + str(heightValuetoGet) + '.png', dpi = 300)
        plt.show()
        ...
        BACRegionRange=list(range(BACFirstRegion,BACLastRegion+1))
        WithoutBACRegionRange=list(range(WithoutBACFirstRegion,WithoutBACLastRegion+1))       
        ...
        for BACregion in BACRegionRange:
            ...
            RefBACArea=io.imread(diretorio + importstackRootName + '/With_BAC/t' + str(BACstackNumber) + '/Region' + str(BACregion) + '/ZProjectionBAC.png')            
            BACAreaList.append(float(np.count_nonzero(RefBACArea.flatten()) * AreaPixelRatio))
            
            for sliceN in SliceRange:
                a11=io.imread(diretorio + importstackRootName + '/With_BAC/t' + str(BACstackNumber) + '/Region' + str(BACregion) + "/Slice" + str(sliceN) + ".png")
            ...
            if Det_TopoXVol_relation == True:
                with open(diretorio + importstackRootName + '/With_BAC/t' + str(BACstackNumber) + '/Region' + str(BACregion) + "/Bac_Vol.txt", 'r') as BacVolValue_str:
        ...
        if not os.path.exists(diretorio + importstackRootName + '/WithoutBAC_SaList_t1' + '.txt'):
            for WithoutBACregion in WithoutBACRegionRange:
                ...
                for sliceN in SliceRange:
                    b11=io.imread(diretorio + importstackRootName + '/WithoutBACScaled' + '/t' + str(SUFstackNumber) + '/Region' + str(WithoutBACregion) + '/Slice' + str(sliceN) + '.png')
                ...
                if float(ImglistWithoutBACF[0].shape[0] * ImglistWithoutBACF[0].shape[1]) > MaxScaledAreaToConsider:
                    ...
                    File4=open(diretorio + importstackRootName + '/WithoutBAC_SaList_t' + str(SUFstackNumber) + '.txt', 'a')
                    File5=open(diretorio + importstackRootName + '/WithoutBAC_SqList_t' + str(SUFstackNumber) + '.txt', 'a')
                    File6=open(diretorio + importstackRootName + '/WithoutBAC_SsKList_t' + str(SUFstackNumber) + '.txt', 'a')
