...
def AnalyzeMergedZones(diretorio,
    ...
    plt.tight_layout()    
    fig1.savefig(diretorio + '/Imagens/MergedRegion' + str(MergedRegion) + '.png', dpi = 300)
    #fig2.savefig(diretorio + '/Imagens/MergedRegion' + str(MergedRegion) + '_PLOT.png')
    plt.show()
...
def AvgVelocity(volumeValsArray, initialTimePoint, timeinterval, element):
    ...
    for volVal in list(range(volumeValsArray.shape[0]-1)):
        VelocList.append([element, time + (timeinterval/2), (volumeValsArray[volVal] + volumeValsArray[volVal+1])/timeinterval])
        time += timeinterval
