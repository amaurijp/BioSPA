...
def VolAnalysis(ExperimentNumber,
    ...
    if RegionAnalysis == False:
        ...
        for stackNumber in StackList:
            ...
            ProjectImageBAC=io.imread(diretorio + ImportRootFolder + '/t' + str(stackNumber) + '/ZProjection.png')
        ...
        try:
            ...
            fig1.savefig(diretorio + '/Images/VolumeCurveFull.png', dpi = 300)
            plt.show() 
    ...
    if RegionAnalysis == True:        
        ...
        Lastregion = len(os.listdir(diretorio + '/ExportedData/VolumeValues')) - LastStack
        ...
        print('\n\nThe median of a value is ', np.median(np.array(BACExpAList_1)))
        ...
        print('\n\nThe median of b value is ', np.median(np.array(BACExpBList_1)))
        ...
        #salvando
        fig1.savefig(diretorio + '/Images/Fitting1.png', dpi = 300)
        fig2.savefig(diretorio + '/Images/Fitting2.png', dpi = 300)
        fig3.savefig(diretorio + '/Images/Fitting3.png', dpi = 300)
        fig4.savefig(diretorio + '/Images/Fitting4.png', dpi = 300)
        
        plt.show()
        
        print('\n\n\nIt was fitted ', fittedPerc_1,' of the ', len(RegionList),' elements found.\n')
        ...
        print('\n\n\nIt was fitted ', fittedPerc_N,' of the ', len(RegionList),' elements found (p0 offset).\n')
        ...
        print('\n\n Single Cells, ', len(List_singlecells), ' units', '(', len(List_singlecells)/len(BACVolList_p0) * 100, '%) \n')
        print('\n\n Clusters, ', len(List_cluster), ' units', '(', len(List_cluster)/len(BACVolList_p0) * 100, '%) \n')
        print('\n\n Aggregates, ', len(List_aggregates), ' units', '(', len(List_aggregates)/len(BACVolList_p0) * 100, '%) \n')
        print('\n\n Microcolonies, ', len(List_microcolonies), ' units', '(', len(List_microcolonies)/len(BACVolList_p0) * 100, '%) \n')
        
        print('\n\nTotal of colonizers = ', len(BACVolList_p0))
        
        #Export txt P0_X_ExpB
        file1=open(diretorio + '/ExportedData/P0_X_ExpB_' + str(ExperimentNumber) + '.txt', 'w')
        ...
        file6=open(diretorio + '/ExportedData/NoFIT_V0_X_BacFinalVol_' + str(ExperimentNumber) + '.txt', 'w')
        for Number in list(range(len(NoFitBacMinMax))):
            file6.write(str(NoFitBacMinMax[Number]) + '\n')
        file6.close()
        
        #Export txt Volume Inicial_X_Volume Final (420 min) para os dados não fitados
        file7=open(diretorio + '/ExportedData/p0_Values_' + str(ExperimentNumber) + '.txt', 'w')
        for Number in list(range(len(BACVolList_p0))):
            file7.write(str(BACVolList_p0[Number]) + '\n')
        file7.close()        

        file8=open(diretorio + '/ExportedData/MinVol_X_MaxVol_1_' + str(ExperimentNumber) + '.txt', 'w')
