...
def ATDETAnalysis(SettingsDic):    
    ...
    MapField=[SettingsDic['height_field'], SettingsDic['width_field']] # [DeltaX, DelatY] in microns
    MaxTimePoint=SettingsDic['timePoints']
    TimeInterval=SettingsDic['timeinterval']
    
    
    ATData=io.imread(diretorio + '/Images/TimeAnalysis_AT.png')
    ATticks=[]
    ...
    DETData=io.imread(diretorio + '/Images/TimeAnalysis_DeT.png')
    DETticks=[]
    for point in list(np.arange(0,255+1,255/MaxTimePoint)):
    ...
    fig1=plt.figure(figsize=(10, 7), facecolor='w', edgecolor='k')
    
    #Attachement map
    map1=plt.subplot2grid((1,2),(0,1),rowspan=1,colspan=1)
    map1.set_title('Bacteria attachment and growth map',fontsize=12)
    ...
    map1.set_xlabel('$\\mu$m', fontsize=12)
    ...
    Colbar1.set_label('Time (min)', fontsize=12)
    Colbar1.set_ticks(list(np.arange(0,max(ATticks)+1,(255/(MaxTimePoint+1.5)))))
    Colbar1.set_ticklabels(['B']+[str(i) for i in ATticks])
    
    
    #Detachement map
    map2=plt.subplot2grid((1,2),(0,0),rowspan=1,colspan=1)
    map2.set_title('Bacteria Detachment map', fontsize=12)
    ...
    map2.set_xlabel('$\\mu$m', fontsize=12)
    ...
    Colbar2.set_label('Time (min)', fontsize=12)
    Colbar2.set_ticks(list(np.arange(0,DETData.max()+1,(255/MaxTimePoint))))
    Colbar2.set_ticklabels(['B']+[str(i) for i in DETticks[1:]])
    ...
    plt.tight_layout()
    fig1.savefig(diretorio + '/Images/TimeColonization.png', dpi = 300)
    plt.show()